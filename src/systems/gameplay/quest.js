...
class QuestManager {
    ...
    planExpedition(locationId) {
        if (!this.areExpeditionsUnlocked()) {
            window.showToast(
                ...
                { type: 'warning', icon: '⚠️', timeout: 4000 }
            );
            return false;
        }
        ...
        if (!location || !location.unlocked) {
            window.showToast('Location not available for expedition!', { type: 'warning', icon: '⚠️', timeout: 3000 });
            return false;
        }
        ...
        if (availableRoyals.length === 0) {
            window.showToast(
                ...
                { type: 'warning', icon: '👑', timeout: 4000 }
            );
            return false;
        }
        ...
        if (window.modalSystem) {
            ...
            document.getElementById('confirm-expedition-btn').addEventListener('click', () => {
                ...
                if (selectedLeader) {
                    ...
                    window.showToast('Please select an expedition leader!', { type: 'warning', icon: '⚠️', timeout: 3000 });
                }
            });
    ...
    startExpedition(locationId, leaderId = null) {
        ...
        if (this.expeditionState !== 'none') {
            window.showToast('Army is already on an expedition!', { type: 'warning', icon: '⚠️', timeout: 3000 });
            return;
        }
        ...
        if (leaderId) {
            ...
            if (!expeditionLeader) {
                window.showToast('Selected royal leader is not available!', { type: 'warning', icon: '👑', timeout: 3000 });
                return;
            }
        }
        ...
        if (!this.canStartExpedition(location, availableSupplies, requiredSupplies)) {
            window.showToast('Insufficient supplies for expedition!', { type: 'warning', icon: '📦', timeout: 3000 });
            return;
        }
    ...
    generateTravelEvent() {
        ...
        window.showToast(event.text, {
            type: event.type === 'positive' ? 'success' : event.type === 'negative' ? 'warning' : event.type === 'challenge' ? 'error' : 'info',
    ...
    restArmy() {
        if (this.currentExpedition.supplies.food >= 5) {
            ...
            this.renderTravelView();
            window.showToast('Army morale improved!', { type: 'success', icon: '😊', timeout: 3000 });
        } else {
            window.showToast('Not enough food supplies to rest!', { type: 'warning', icon: '⚠️', timeout: 3000 });
        }
    }
    ...
    forceMarch() {
        ...
        this.renderTravelView();
        window.showToast('March pace increased!', { type: 'info', icon: '⚡', timeout: 3000 });
    }
    ...
    huntForFood() {
        ...
        if (huntSuccess) {
            ...
            window.showToast(`Hunt successful! +${foodGained} food`, { type: 'success', icon: '🏹', timeout: 3000 });
        } else {
            this.currentExpedition.events.push({
                type: 'neutral',
            ...
            window.showToast('Hunt failed - no game found', { type: 'warning', icon: '🚫', timeout: 3000 });
        }
    ...
    completeExpedition() {
            ...
            Object.keys(rewards).forEach(resource => {
                if (this.gameState.resources[resource] !== undefined) {
                    ...
                    if (this.gameState.populationManager?.addRefugees) {
                        ...
                        if (added < planned && window.showToast) {
                            window.showToast(`🏠 Housing full: accepted ${added}/${planned} refugees`, { type: 'warning', icon: '⚠️', timeout: 4000 });
                        }
                    } else {
        ...
        window.showToast(
            ...
            { type: isSuccessful ? 'success' : 'error', icon: resultIcon, timeout: 6000 }
        );
        ...
        if (leader.id === 'monarch' && this.gameState.monarch) {
            ...
            if (this.gameState.monarch.expeditionExperience >= 50) {
                ...
                window.showToast(
                    ...
                    { type: 'success', icon: '📈', timeout: 4000 }
                );
            }
        } else if (this.gameState.populationManager?.royalFamily) {
    ...
    injureRoyalLeader(leader) {
        ...
        window.showToast(
            ...
            { type: 'warning', icon: '🏥', timeout: 5000 }
        );
    }
    ...
    showExpeditionSummary(location, rewards, result) {
        ...
        if (window.modalSystem) {
            ...
            window.showToast(summaryHTML, { type: result === 'success' ? 'success' : 'error', icon: result === 'success' ? '🎁' : '💀', timeout: 12000 });
        }
    }
    ...
    showExpeditionSummary(location, rewards) {
        ...
        window.showToast(summaryHTML, { type: 'success', icon: '🎁', timeout: 8000 });
    }
